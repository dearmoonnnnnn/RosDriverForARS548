...
这里在`ars548_process`这个包中自定义一个消息类型。

```yaml
# 在你的 ROS 包中创建一个名为 `DopplerPoint.msg` 的消息文件

# 文件路径: RosDriverForARS548/src/ars548_process/msg/DopplerPoint.msg
...
- 使得`ars548_process`可以找到`geometry`包
...
```c++
#include "ars548_process/DopplerPoint.h"  // 包含自定义消息类型

void detectionReceive(const ars548_process::DetectionList& msg)
{
    uint size = msg.detection_array.size();

    ars548_process::DopplerPoint doppler_point;  // 自定义消息类型
...
##### 报错：无法将`ars548_process::DopplerPoint` 类型的对象添加到 `std::vector<geometry_msgs::Point32>`类型的容器中
...
#include <ars548_process/DetectionList.h> // 假设这是雷达消息类型
...
2）在ars548_process下创建include文件夹
